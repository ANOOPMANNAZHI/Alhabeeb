...
<div class="page-bar">
    <div class="page-title-breadcrumb">
        ...
        @if($sales_enquiry->work_flow_processes_code == 203)
			{{Breadcrumbs::render('contractApprovalListInfo',$id,$sales_enquiry->work_flow_processes_code)}}
        @else
			{{Breadcrumbs::render('contractApprovalListInfoWon',$id,$sales_enquiry->work_flow_processes_code)}}
        @endif
    </div>
</div>
<div class="row">
...
<div class="card card-box salesLeadBox">
    <div class="card-body">
            <input type="hidden" id="enquiryid" name="enquiryid" value="{{$landlordContractInfo->sale_enquiry_id}}">
          
            <input type="hidden" id="workflow_id" name="workflow_id" value="{{$sales_enquiry->work_flow_processes_code}}">
            @if($sales_enquiry->work_flow_processes_code == 203)
               <button type="submit" class="btn btn-circle btn-primary align-right accept" data-toggle="modal" data-target="#myModal" data-id = "ACPT"  data-backdrop="static" data-keyboard="false"> Approve </button>
                <!-- ---------------------- Reject Button Active only for Stage wise contract ----------  -->
                @if($direct_indirect_status == false && $sales_enquiry->sales_enquiry_direct_contract == 1)
                    <button type="button" class="btn btn-circle btn-primary align-right reject" data-toggle="modal" data-target="#myModal" data-id = "RJCT" data-backdrop="static" data-keyboard="false"> Reject </button>
                @endif
				...
				<a title="Edit" href="{{route('landlord-contract.edit',$landlordContractInfo->id)}}" class="btn btn-circle btn-primary align-right" >
					Edit
				</a>
				@endcan
            @endif
        ...
        <div class="col-sm-6 p-0">
            <dl>
                ...
                <dt>Email</dt>
                <dd><a href="mailto:{{$sales_enquiry->sales_email}}">{{$sales_enquiry->sales_email??'NA'}}</a></dd>
                <dt>Phone</dt>
                ...
                @if(isset($sales_enquiry->sales_notes))
                    <dt>Reason</dt>
                    <dd>{{$sales_enquiry->sales_notes??'NA'}}</dd>
                @endif
                ...
                <div class="row">
                    ...
                    <div class="col leadInformation">
                        <ul>
                            ...
                            <li>{{$sales_enquiry->salesEnquiryUser->username}}</li>
       ...
       <div class="table-responsive1">
        <table class="table" >
          ...
          @forelse ($salesNoteList as $note)
            ...
            @if(!empty($note->sales_notes)) 
            <tr>
			  <td>{{$note->workFlowProcess->work_flow_processes_name}} </td>
              <td>{{$note->sales_notes}}</td>
              <td>{{$note->createdBy->employee->employee_name?$note->createdBy->employee->employee_name:$note->createdBy->username}}</td>
...
<div class="card card-box salesLeadBox">
    ...
    <div class="card-body row">
        ...
        <div class="col-lg-6 p-t-20"> 
            <div class = "txt-full-width">
               <h5 class="details"><b>Agreement Amount :  </b><span>{{numberFormat($landlordContractInfo->landlord_contract_agreement_amt)}} OMR</span></h5>
            </div>
        </div>
        @endif
...
$(document).ready(function() {
    ...
    $('.reject, .accept').on('click', function(e) {        
