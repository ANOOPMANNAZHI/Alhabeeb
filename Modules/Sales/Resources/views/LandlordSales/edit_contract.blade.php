...
<div class="page-bar">
    <div class="page-title-breadcrumb">
        <div class=" pull-left">
            <div class="page-title">Edit Contract Details</div>
        </div>
        {{Breadcrumbs::render('landlordContract.edit',$landlordContract->id)}}
    </div>
</div>
<div class="row">
<div class="col">
<div class="card card-box salesSearchBox">
 <form autocomplete="off" action="{{route('landlordContract.update',$landlordContract->id)}}" method="POST" id="form_sample_2" class="form-horizontal" enctype="multipart/form-data" data-toggle="validator">
...
<div class="dataSearchBox sub-box ">
        ...
        <div class="row">
            <div class="col-sm-6">
            <div class="form-group">
                <label for="vendor_id">Landlord Name</label>
                 <div class="p-relative">
                    <i class="fa fa-terminal icn-add" aria-hidden="true"></i>
            ...
            <div class="form-group">
                <label for="management_method">Management Fees</label>
                 <div class="p-relative">
						...
						<input type="radio" name="management_method" value ="1" {{ isset($landlordContract->management_method)? (($landlordContract->management_method==1)?'CHECKED':''):'CHECKED' }} class="management_method"> Percentage
						<input type="radio" name="management_method" value ="2" {{ isset($landlordContract->management_method)? (($landlordContract->management_method==2)?'CHECKED':''):'' }} class="management_method"> Amount
            </div>
            </div>
        </div>
       <div class="w-100"></div>
...
<div class="dataSearchBox sub-box ">
        ...
        <div class="col-sm-6">
            <div class="form-group">
                <label for="landlord_duration">Duration Type</label>
                 <div class="p-relative">
                    <i class="fa fa-calendar-o icn-add" aria-hidden="true"></i>
					<select class="form-control" name="duration_type" id="duration_type" required="">
						<option value="">Select Duration Type </option>
						<option value="1" {{ isset($landlordContract->landlord_contract_duration)? (($landlordContract->landlord_contract_duration==1)?'SELECTED':''):'' }}
						{{isset($landlordContract->landlord_contract_duration)?   (($landlordContract->management_id==1)?'DISABLED':''):'' }}>Open
						</option>
        ...
        <div class="col-sm-6">
            <div class="form-group">
                ...
                <select class="form-control" name="landlord_marketing_executive" >
                    ...
                    @foreach($salesTeam as $emp)
                        @if(isset($emp->employee->id))
                        <option {{isset($landlordContract)?(($landlordContract->landlord_marketing_executive == $emp->employee->id)?'selected':''):''}} value={{$emp->employee->id}}>{{$emp->employee->employee_name.'('.$emp->employee->employee_code.')'}}</option>
                        @endif
                    @endforeach
                </select>
                </div>
