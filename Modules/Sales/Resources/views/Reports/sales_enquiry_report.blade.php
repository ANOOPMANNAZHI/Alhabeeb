...
<div class="page-bar">
  <div class="page-title-breadcrumb">
    <div class=" pull-left">
      <div class="page-title">PLMS Sales Enquiries</div>
    </div>
...
<div class="row">
  <div class="col">
    <div class="card card-box salesSearchBox">
      ...
      <form action="{{route('salesEnquiryReportPdf')}}" target="_blank" method="POST" id="form_sample_2" class="form-horizontal" enctype="multipart/form-data" data-toggle="validator">
        {{csrf_field()}}
        <div class="dataSearchBox ">
                ...
                <div class="p-relative">
                 ...
                 <select class="form-control" id="unit_type"  name="unit_type">
                  ...
                  @foreach($unitTypes as $utype)
                     <option value="{{$utype->unit_types_name}}">{{$utype->unit_types_name}}</option>  
                   @endforeach                
                </select> 
              </div>               
            </div>
                ...
                <div class="p-relative">
                 ...
                 <select class="form-control" id="location"  name="location">
                  ...
                  @foreach($locations as $loc)
                     <option value="{{$loc->locations_name}}">{{$loc->locations_name}}</option>  
                   @endforeach   
          ...
          <div class="col-sm-6">
              <div class="form-group">
                <label for="location">Enquiry Source</label>
                <div class="p-relative">
                 <i class="fa fa-cubes icn-add" aria-hidden="true"></i>
                 <select class="form-control" id="source"  name="source">
                  <option value="">All</option>                
                     @foreach($enquirySources as $source)
                     <option value="{{$source->enquiry_sources_name}}">{{$source->enquiry_sources_name}}</option>  
                     @endforeach     
                </select> 
              </div>               
            </div>
                ...
                <div class="p-relative">
                 ...
                 <select class="form-control" id="assignedto"  name="assignedto">
                  <option value="">All</option>                
                     ...
                     <option value="{{$user->employee->employee_name}}">{{$user->employee->employee_name}}</option>  
                     @endforeach     
                </select> 
              </div>               
            </div>
