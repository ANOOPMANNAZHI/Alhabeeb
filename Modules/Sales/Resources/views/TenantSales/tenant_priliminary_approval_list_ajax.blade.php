                      @php $count = 1; @endphp
                                @forelse ($preliminaryApprovalLists as $approval_list)
                                @php
                                  $current = 'preliminaryApprovalList';
                                ...
                                <tr>
                                    <td><a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">
                                        {{$approval_list->sales_enquiry_no}}

                                    </a></td>
                                    <td><a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">{{$approval_list->created_at->format('d/m/Y')}}</a>
                                    </td>
                                    <td><a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">{{$approval_list->sales_enquiry_name}}</a>
                                    </td>
                                    
                                   <td><a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">
                                      {{$approval_list->agre_build}}
                                    </a></td>
                                   <td>
                                    <a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">

                                       {{$approval_list->agre_unit}}
                                     </a>
                                   </td>
                                    <td><a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">
                                    ...
                                    <td><a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">
                                         {{$approval_list->agre_start}}
                                    </a></td>
                                    <td><a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">
                                        {{$approval_list->agre_rent}}
                                    </a></td>
                                    <td>
                                      
                                    @if($approval_list->employee_name)
                                    <a class="no-link" href="{{route('leadAssign.nextStage',[$approval_list->sales_enquiry_id,$approval_list->sales_work_flow])}}">
                                      {{$approval_list->employee_name}}
                                    </a>
                                    @endif
                                    </td>
                                    <td align="center">
                      ...
                      <tr>                   							          
                       <td colspan="5" id="pagination_ajax">
								        {{$preliminaryApprovalLists->withPath($route)->appends(\Request::except(['page','_token','ajax']))->links()}}
