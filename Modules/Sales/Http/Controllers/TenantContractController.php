...
class TenantContractController extends Controller
{
     ...
     //       $contractCode = $prefix.str_pad($tenantContractLatest->id+1,4,'0',STR_PAD_LEFT);
      //  else
        ...
        $this->validate($request, [
            ...
            'resident_id' => 'required_if:tenant_type_id,1',
            'nationality_name' => 'required_if:tenant_type_id,2|required_if:selection,3|required_if:selection,4',
            'nationality_name' => 'required_if:tenant_type_id,1',
            'tenant_contract_payment_type' => 'required', 
         ]);
        ...
        $market=User::role('sales_person')->with('employee')->where('id',$request['tenant_marketing_executive'])->first();
        $tenantContract =  TenantContract::create([
          ...
          'tenant_marketing_executive'=>$market->user_type_id,
          'tenant_contract_duration_countdown' => $request['yeartxt'].'-'.$request['monthtxt'].'-'.$request['daytxt'],
          'tenant_contract_value' => replaceCommaWithDot($request['tenant_contract_value']),
          'deposit_check'=> $request['deposit_check'],
          'partial_comment'=> $partial_comment,
          'created_by' => \Auth::user()->id,
        ]);
		Unit::where('id',$request['unit_id'])->update(['unit_vaccant_status'=>1,'unit_status'=>1]);
    ...
    {
	    $market=Employee::where('id',$tenantContract->tenant_marketing_executive)->first();
        return view('sales::TenantSales.tenant_view_documentation_modal',compact('tenantContract','market'));
    }
    ...
    public function edit(TenantContract $tenantContract)
    {
		...
		$employee     = User::role('sales_person')->with('employee')->where('user_type_id',$tenantContract->tenant_marketing_executive)->first();
    ...
    public function update(Request $request,TenantContract $tenantContract)
    {
      //dd($request->all());
       ...
       //dd( $tenant->id);
        ...
        $durationInMonth = $yearToMonth + $request['monthtxt'];
		$market=User::role('sales_person')->with('employee')->where('id',$request['tenant_marketing_executive'])->first();
        $oldunit_id = $tenantContract->unit_id;
        ...
        Unit::where('id',$oldunit_id)->update(['unit_vaccant_status'=>0]);
        Unit::where('id',$request['unit_id'])->update(['unit_vaccant_status'=>1]);
...
$contractData['tenant_contract_no'] = request('tenant_contract_no');
    ...
    */
    public function contractCreation($id,$stage) {
      ...
      return view('sales::TenantSales.tenant_contract_create',compact('tenantContract','stage','banks','locations','employee','employeeList','rent','duration','vaccant_units','fromContract','toContract','upload_size','isYearCorrect'));
  ...
  //dd($month);
       ...
       /* $pastTwoMonthDt = date('Y-m-d', strtotime("-3 month", strtotime(date('Y-m-d'))));
        ...
        if(!empty($sales_enquiry_id)){
          $createdUnit = TenantContract::where('sale_enquiry_id',$sales_enquiry_id)
                ->pluck('unit_id');
          $units['buildUnit'] = Unit::where('building_id',$id)->where('unit_status',1)->where('unit_vaccant_status',0)
                ->when($createdUnit, function ($query, $createdUnit) {
                    return $query->whereNotIn('id', $createdUnit);
                })->orderBy('unit_no','asc')->get();
        }else{

          $units['buildUnit'] = Unit::where('building_id',$id)->where('unit_status',1)->where('unit_vaccant_status',0)->orderBy('unit_no','asc')
                ->get();
        }
    ...
    */
    public function buildingByUnitWithoutCheckVaccant(Request $request){
	   ...
	   $units['buildDimCode']  = Building::where('building_status',1)->where('id',$id)->first();
