...
class ViewEnquirySalesPerson extends Model
{
	use Sortable;
	protected $guarded = [];
	protected $table   = 'salesperson_enquiry';
	public $sortable   = ['sales_enquiry_no','fc_att', 'cust', 'cust_no','unit_type','loc','building_name','unit_code','unit_type','created_at','enquiry_flow'];
   ...
   */
   public function scopeFilter($query, $request){
      ...
      $generalSearch = array();   
        ...
        foreach($request->except(['_token','list_by_week','sort','direction','page','curr_url','fieldValues','ajax','operation','logic','route','fc_att','params','from','to','open_enquiry','created_by']) as $key => $val){
...
public function scopeOpenEnquiry($query,$request){
 
  if(isset($request->open_enquiry)){
    $query->where('work_flow_processes_code', '!=',108)->where('work_flow_processes_code', '!=',109);
    return $query;
  }
  return $query;
}
