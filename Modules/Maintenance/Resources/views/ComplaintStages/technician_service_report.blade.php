...
<div class="row">
 ...
 @if($ticket->complaintServiceReport->complaint_assign_status == 3  && $ticket->complaintChecklist->complaintEnquiry->complainer_category == 1)
 <div class="col-md-12">
  <div class="card-box">
    <div class="card-body">
      <div class="col-sm-4 tenantNameClass">
        <b>Tenant Name: </b>{{$ticket->complaintChecklist->complaintEnquiry->complainer_name}}
        <input type="hidden" name="complainer_name" id="complainer_name" value="{{$ticket->complaintChecklist->complaintEnquiry->complainer_name}}">
      </div>

      <iframe name="my_iframe" src="{{route('signature')}}" scroll="none" style="overflow: hidden;height: 350px; width:350px"></iframe>
      <input type="hidden" name="tenantId" id="tenantId" value="{{$ticket->complaintChecklist->complaintEnquiry->tenant_id}}">

      @foreach($tickets as $assigned_ticket)
      <input type="hidden" name="assigned_ticket[]" value="{{$assigned_ticket}}">
      @endforeach
      <input type="hidden" name="complaint_report_id" id="complaint_report_id" value="{{$ticket->complaint_service_report_id}}">
     ...
     <div class="card-head">
      <h4>
        ...
        @endif
         ...
         @role('technician')
         @if($ticket->complaintChecklist->complaintEnquiry->complainer_category ==2 or $ticket->complaintChecklist->complaintEnquiry->complainer_category ==3)
          <a title="Closed" href="{{route('serviceReportStatusButtonUpdate',[$ticket->complaint_service_report_id,4])}}" class="btn btn-circle btn-primary float-right" >Closed</a>
           @endif
         @endrole
        ...
        <div class="col-md-6 p-t-10">
          <div class="row">
            ...
            <div class="col-md-6"><span>
              ...
              {{$ticket->complaintChecklist->subAssignedPerson->employee->employee_name ?? $ticket->complaintChecklist->subAssignedPerson->username}}
              @endif
            </span></div>
          </div>
        </div> 
   ...
   <div class="card-body">
    <div class="dataSearchBox">
      <div class="add-note-section">
       ...
       <form  id="sales_note-form" autocomplete="off" action="{{route('storeServiceReportNote')}}" class="sales_note_modal" method="POST" enctype="multipart/form-data" data-toggle="validator">
         {{csrf_field()}}
         <div class="col-sm-12">
...
<div class="col-md-12">
  <div class="card-box">
   <div class="card-body">
    <div class="dataSearchBox ">
      <form autocomplete="off" action="{{route('complaintStage.store')}}" method="POST" id="item_form"" class="form-horizontal service_item_note" enctype="multipart/form-data" data-toggle="validator">
        {{csrf_field()}}
        <div><h4>Item</h4></div>
        <div class="row">
          <div class="col-sm-3">
       ...
       <div class="col-sm-1">
        ...
        <button type="submit" class="btn btn-primary add_note">Add</button>
      </div>
    </div>
  </form>
</div>
