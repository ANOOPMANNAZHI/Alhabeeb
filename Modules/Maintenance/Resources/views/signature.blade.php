...
<style type="text/css">
    ...
    <form id="save_remote" method="POST" action ="{{route('generateServiceReport')}}"method="POST"  class="form-horizontal" enctype="multipart/form-data" data-toggle="validator" target="my_iframe" >{{csrf_field()}}
      ...
      style="position: relative; margin: 0 0 10px 0; width:276px; height:100px;float: left; padding: 0; border: 1px solid #c4caac;"></canvas>
        ...
        <div class="col-sm-4">
          <button type="button" class="btn btn-primary btn-spacing" onclick="signatureClear()">Clear Signature</button>
      ...
      var testSignature = $("#testSignature").val();
        if(testSignature != ""){
          $.ajax({
            ...
            url: "{{route('generateServiceReport')}}",
