...
$(document).ready(function() {
	var timer, delay = 500;
	$(document).on('change keyup paste','.search_fields',function(e){ 
 	  var _this = $(this);
 	  var focus 				= $(this).attr('id'); 
	  var complaint_no 			= $("#complaint_no").val();
	  var complaint_date 		= $("#created_at").val();
	  ...
	  $.ajax({
			  method: "GET",
			  url: '{{url()->current()}}',
			  data: { 'complaintEnquiry__complaint_no': complaint_no,'complaintEnquiry__complaint_date': complaint_date,'complaintEnquiry__complainer_name': complainer_name,'complaintEnquiry__complaint_mob_no': complaint_mob_no,'complaintEnquiry__Unit__unit_code': unit,'location__locations_name': location,'route':route,"_token" : $('meta[name="csrf-token"]').attr('content') , 'ajax' : true ,'fieldName' : fieldName , 'operation' : operation , 'fieldValue' : fieldValue , 'logic' : logic ,'building_name':building_id,'ticket_status':status,'complaintTicketsAll__complaint_ticket_no':ticket_no ,'complaintTicketsAll__work__works_code':category,'tenant_status':tenant_status,	'service_report_no':service_report_no ,'priority_status':priority_status,'assigned_to':assigned_to,'assignedPerson__employee__employee_name':assignedPerson,
			      'subAssignedPerson__employee__employee_name':subAssignedPerson,},		  
			  beforeSend: function(){
                // Show image container
                $('#enquiry-search').html("<tr><td colspan='9' align='center'><img src='{{url('/')}}/public/img/pre-loader.gif' width='75' height='75'></td></tr>");
