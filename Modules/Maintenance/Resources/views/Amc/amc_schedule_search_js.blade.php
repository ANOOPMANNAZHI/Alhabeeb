...
$(document).ready(function() {
	...
	$(document).on('change keyup paste','.search_fields',function(){ 
	  ...
	  var amc_contract_type = $("#amc_contract_type").val();
	  
	  var route 				= $("#route").val(); //alert(route);
      var route_href 			= $("#url_route").val();
      var amc_schedule_status 			= $("#amc_schedule_status").val();
	  ...
	  $.ajax({
			  method: "GET",
			  url: '{{url()->current()}}',
			  data: { 'amc_contract_id': amc_contract_id,'amc_schedule_period_from': amc_schedule_period_from,'amc_schedule_period_to': amc_schedule_period_to,'amc_schedule_status':amc_schedule_status,'building__building_name': building_id,'amc_contract_type':amc_contract_type,'vendor__vendor_name': vendor_id,'paymentMethod__payment_method_code': payment_method_id,'route':'{{url()->current()}}',"_token" : $('meta[name="csrf-token"]').attr('content') , 'ajax' : true ,'fieldName' : fieldName , 'operation' : operation , 'fieldValue' : fieldValue , 'logic' : logic },			  
			  beforeSend: function(){
			  ...
			  success: function(data){ 
				...
				var txt = 'amc_contract_id='+ amc_contract_id+'&amc_schedule_period_from='+ amc_schedule_period_from+'&amc_schedule_period_to='+ amc_schedule_period_to+'&building__building_name='+ building_id+'&vendor__vendor_name='+ vendor_id+'&paymentMethod__payment_method_code='+ payment_method_id+'&amc_contract_type='+amc_contract_type+'&amc_schedule_status='+amc_schedule_status;
