...
<div class="page-bar">
  <div class="page-title-breadcrumb">
    <div class=" pull-left">
      <div class="page-title">Termination Approval View</div>
    </div>
    {{ Breadcrumbs::render('tenantTerminationApprovalView',$tenantContract) }}
  </div>
</div>
        ...
        <h4>
                ...
                @if($termination->termination_type_status == 2)
                    @can('tenant_termination_approve')
                        <a href="{{--route('tenantTerminationOpenStatus',[$termination->id,3])--}}" title="Approve" class="btn btn-circle btn-primary align-right approval_note" data-id ="approve" id="{{$termination->id}}"  data-toggle="modal" data-target="#myModal">
                          Approve
                        </a>
                    @endcan
                    @can('tenant_termination_reject')
                        <a href="{{--route('tenantTerminationOpenStatus',[$termination->id,4])--}}" title="Reject" class="btn btn-circle btn-danger align-right reject_note" data-id = "RJCT" id="{{$termination->id}}"  data-toggle="modal" data-target="#myModal">
                          Reject
                        </a>
                    @endcan   
                @endif
            ...
            <table class="table" >
              ...
              <tbody>
                ...
                <tr>
                    ...
                    <td>{{numberFormat($otherDuesCollection->receipts_generation_amt)}}</td>
    ...
    // $("#termination_takenover_date,#termination_date,#termination_remark").on('change input',function(e){
     $(".saveTerminationbtn, .saveTakenOverbtn,.saveRemarkbtn").on('click',function(e){
        ...
        if(clsName=='saveTerminationbtn' && termination_takenover_date_txt==''){
            ...
            alert("TakeOver Date Should Be Less Than Or Equal Termination Date");
            return false;
        }
            ...
            success: function(data)
            {
               if(termination_takenover_date && clsName=='saveTakenOverbtn'){
				...
				if(new Date(termination_takenover_date)  >= new Date(termination_date_dt)){
                    ...
                    alert("Termination Date reseted Due To TakeOver Date Than Greater Termination Date");
                }
				$(".closeTaken").show();
				$(".openTaken").hide();
				$(".closeTakenOverbtn").hide();
...
/****************************************************************************/
    ...
    $(document).on('click','.approval_note, .reject_note', function(e) {        

            var action_key    = $(this).attr('data-id');
            var terminatedId  = $(this).attr('id');
          
            if(action_key == 'approve')
               var action_key =  3;
            else if(action_key == 'RJCT')
               var action_key =  4;
            else{
            ...
            $.ajax({
                ...
                url: "{{route('terminateApproveOrRejectModal')}}", // This is the url we gave in the route
                data: {'terminatedId' : terminatedId,'action_key' : action_key,"_token": "{{ csrf_token() }}"}, // a JSON object to send back
