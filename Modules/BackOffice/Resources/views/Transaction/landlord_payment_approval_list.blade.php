...
@section('search_url', route('landlordPaymentApproval')) 
@section('search_reset', route('landlordPaymentApproval')) 
...
<div class="page-bar">
  <div class="page-title-breadcrumb">
    <div class=" pull-left">
      <div class="page-title">Landlord Payment Approval</div>
    </div>
    {{ Breadcrumbs::render('landlordPaymentApproval') }}
  </div>
</div>
...
<div class="table-wrap">
 <div class="table-responsive">	
      <table class="table display product-overview mb-30" id="dtBasicExample">
        <thead>
          <tr>
            ...
            <th>@sortablelink('landlord_payment_approval_status','Purpose',[],[ 'class' => 'sort_url' ])</th>
            <th>Action</th>
          </tr>
          <tr>
            <td></td>
          ...
          <td>
            <select name="landlord_payment_approval_status" class="searchFields search_fields mob" id="landlord_payment_approval_status" style="width:100px;">
              <option value="">Show All</option>
              <option {{ (old('landlord_payment_approval_status') == 2)? 'selected' : '' }} value="2">{{'Approval'}}</option>
              <option {{ (old('landlord_payment_approval_status') == 3)? 'selected' : '' }} value="3">{{'UnApproval'}}</option>
            </select>
          </td>

          <input autocomplete="off" type="hidden" name="url_route" id="url_route"  value="{{route('landlordPaymentApprovalSearch')}}" ></td>
          <td></td>
        </tr>
      </thead>
      <tbody id="enquiry-search">

        @include('backoffice::Transaction.landlord_payment_approval_list_ajax')  
  ...
  {{$landlordPaymentApprovals->appends(\Request::except(['page','_token','ajax']))->links()}} 
</div> 
...
<script>   
  ...
  jQuery(document).ready(function() {
    ...
    jQuery(document).on('click','.delete_type', function(event) {       
