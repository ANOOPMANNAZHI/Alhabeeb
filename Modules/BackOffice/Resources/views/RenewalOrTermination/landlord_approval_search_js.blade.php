...
$(document).ready(function() {
	...
	$(document).on('change keyup paste','.search_fields',function(){ 
	  ...
	  $.ajax({
			  method: "POST",
			  url: route_href,
			  data: { 'landlord_contract_no': landlord_contract_no,'landlord_contract_valid_from_date': landlord_contract_valid_from_date,'landlord_contract_valid_to_date': landlord_contract_valid_to_date,'building_id': building_id,'vendor_id': vendor_id,
			  'landlord_contract_amt': landlord_contract_amt,'route':route_href,"_token" : $('meta[name="csrf-token"]').attr('content') , 'ajax' : true ,'fieldName' : fieldName , 'operation' : operation , 'fieldValue' : fieldValue , 'logic' : logic },			  
			  beforeSend: function(){
