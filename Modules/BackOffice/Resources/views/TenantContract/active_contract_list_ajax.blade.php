@php $count = 1; @endphp
        ...
        @endphp 
                  ...
                  <tr>                   
					...
					<td>
              <a class="no-link" @if($editFlag) title="Edit"@endif href="{{url($curr_url)}}">{{$contract->unit_code}}</a>
          </td>
                    <td>
                    ...
                    <td>
                        <a class="no-link"  @if($editFlag) title="Edit"@endif href="{{url($curr_url)}}">@if($contract->tenant_contract_rent){{number_format($contract->tenant_contract_rent,3,",","")}}@endif</a>                      
                     </td>
                     <td>
                     ...
                     <td>
                        ...
                        @if($contract->work_flow_processes_code == '108' && $contract->tenant_contract_is_revoke != 1 )
						...
						@if(!$pendingPage)
                          @if($contract->work_flow_processes_code == 107)  
                          ...
                          <button title = "Accept" type="submit" id="accept" class="btn btn-tbl-general btn-xs accept_flow" data-toggle="modal" data-target="#myModal" data-id = "ACPT" data-enquiryid="{{$contract->sale_enquiry_id}}" data-sales_id="{{$contract->sale_enquiry_id}}" data-workflow_id="{{$contract->work_flow_processes_code}}" data-backdrop="static" data-keyboard="false"><i class="fa fa-check"></i></button>  
                          @endif 
                          @if($contract->work_flow_processes_code == 108)
                          <button title = "Reject" type="submit" id="reject" class="btn btn-tbl-delete btn-xs reject" data-toggle="modal" data-target="#myModal1" data-id = "RJCT" data-enquiryid="{{$contract->sale_enquiry_id}}" data-tenant_contract_id="{{$contract->id}}" data-backdrop="static" data-keyboard="false"><i class="fa fa-window-close-o"></i></button>
                       ...
                       @else
                        ...
                        @can('tenant_contract_send_for_approval')
                          @if($contract->tennat_contract_direct_indirect_status == 1 && $contract->tenant_contract_status == 0 && $contract->work_flow_processes_code != 107 && $contract->tenant_renewal_termination_status != 8) 
                       ...
                       @can('tenant_contract_status')
						   @if($contract->status != 5 && $contract->work_flow_processes_code == 108)
								<button type="button" class="btn btn-tbl-violet btn-xs changeStatus" data-id="{{$contract->id}}" data-toggle="modal" data-target="#myModal" title="Change Status" data-backdrop="static" data-keyboard="false"><i class="fa fa-comments-o" aria-hidden="true"></i></button>  
						   @endif
                       @endcan 
          ...
          @if($contract->tenant_contract_status == 1)  
              ...
              <a  href="{{route('invoice.show',$contract->id)}}" class="btn btn-tbl-view btn-xs" title="{{($contract->invoice_check==1)?'View Invoice':'Generate Invoice'}}">
          ...
          @endif  
                       ...
                       @if($contract->work_flow_processes_code >= 106)
                       @if(!isset($pendingPage))
