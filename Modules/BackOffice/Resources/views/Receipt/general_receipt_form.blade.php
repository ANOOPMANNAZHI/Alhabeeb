...
<form action="{{isset($generalReceiptInfo)?route('updateGeneralReceiptAction',$generalReceiptInfo->id):route('addGeneralReceiptAction')}}" method="POST" class="form-horizontal" enctype="multipart/form-data" data-toggle="validator" id="receipt_generation_form" autocomplete="off">
    ...
    <div class="dataSearchBox">    
        ...
        <div class="col-sm-6">
            ...
            <div class="col-sm-6">
                <div class="form-group">
                    <label for="tenant_contract_no">Agreement From :  <small class="textRed">*</small></label>
                    <div class="p-relative">
                        <i class="fa fa-calendar-o icn-add" aria-hidden="true"></i>
                        <input type="date" name="tenant_contract_effective_date" id="tenant_contract_effective_date" class="form-control" value="{{isset($generalReceiptInfo)?  old('tenant_contract_effective_date',$generalReceiptInfo->tenantContractInfo->tenant_contract_effective_date->format('Y-m-d')): old('tenant_contract_effective_date','')}}" readonly>
            ...
            <div class="col-sm-6">
                <div class="form-group">
                    <label for="tenant_contract_no">Agreement To :  <small class="textRed">*</small></label>
                    <div class="p-relative">
                        <i class="fa fa-calendar-o icn-add" aria-hidden="true"></i>
                        <input type="date" name="tenant_contract_valid_to_date" id="tenant_contract_valid_to_date" class="form-control" value="{{isset($generalReceiptInfo)?  old('tenant_contract_valid_to_date',$generalReceiptInfo->tenantContractInfo->tenant_contract_valid_to_date->format('Y-m-d')): old('tenant_contract_date','')}}" readonly>
               ...
               <div class="form-control" > 	
                ...
                <input type="radio" class="payment_method" name="receipts_generation_payment_method"  	
                  ...
                  Cash 
                   ...
                   <input type="radio" class="payment_method" name="receipts_generation_payment_method"     
                  {{!isset($generalReceiptInfo->receipts_generation_payment_method)?'':'disabled'}} 
                    {{!isset($generalReceiptInfo->receipts_generation_payment_method)?'':(( $generalReceiptInfo->receipts_generation_payment_method=3)?'checked':'')}} required value="3"> 
                  Bank Transfer 
              </div>
          </div>
    ...
    <div class="form-group">
        ...
        <div class="p-relative">
            <i class="fa fa-comment icn-add" aria-hidden="true"></i><span>
                ...
                <textarea class="form-control" name="receipts_generation_remark" id="receipts_generation_remark" rows="3" cols="60" placeholder="Enter Remark" >{{ isset($generalReceiptInfo)?  old('receipts_generation_remark',$generalReceiptInfo->receipts_generation_remark): old('receipts_generation_remark','')}}</textarea>  
            </span>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
         ...
         let path = window.location.href.split('/')[6]
         //console.log(parseInt(path));
         if(path != undefined){

            $.ajax({
                       url: '{!!URL::route('getReceiptDetailsjson')!!}',
                       data: {id:parseInt(path)},
                       success:function(response){

                          var am = response;
                           
                          $("#receipts_generation_amt").val(am);
                    }
    ...
    data: { mobile: mobilenumber, resident_card_id:resident_card_id,
        ...
        success: function(data){
          ...
          $("#ax_division").append('<option value="'+divVal +'">'+ div +'</option>');
            ...
            $('#receipts_generation_description').val('General :'+result[2].building_name+'/'+result[3].unit_no);
        }
...
$("#mob_no, #resident_card_id").on('change',function(event){
    ...
    if(eventEl == 'resident_card_id' || eventEl == 'mob_no'){
        ...
        $('#ax_division').val(''); 
		$('#receipts_generation_description').val('');
    ...
    if(contractId){
        $.ajax({
            ...
            success: function(data){
                ...
                $('#tenant_name').val(result[1].tenant_name);
                $('#tenant_code').val(result[1].tenant_code);
				//$('#receipts_generation_description').val('Uni '+result[3].unit_no+' , '+result[2].building_name);
				$('#receipts_generation_description').val('UNIT NO. '+result[3].unit_no+' , '+result[2].building_name);
            }
        }
    });
    }
...
$(document).on('click','.payment_method',function(){  
    ...
    else if(payment_method == 3){ // Online transfer
...
function copydata(dt){
    $("#receipts_generation_remark").val(dt);
}
