...
public function showDepositRentReport(){
  // Same rule as showDepositRentReportV2: any building with a deposit receipt
  // on any contract, not only on currently-active contracts.
  $buildings =   Building::active()
  ->whereIn('id', function ($query) {
    $query->select('un.building_id')
      ->from('receipts_generation as rg')
      ->join('tenant_contracts as tc', 'tc.id', '=', 'rg.tenant_contract_id')
      ->join('units as un', 'un.id', '=', 'tc.unit_id')
      ->where('rg.receipts_generation_type', 2)
      ->whereNotNull('un.building_id');
...
public function showDepositRentReportV2(){
  // Any building with a deposit receipt on any of its contracts — the receipt
  // may sit on a contract that has since expired/terminated (e.g. Said Sultan
  // Bldg, whose deposits are all on status-0 contracts), and the report query
  // itself does not filter on contract status.
  $buildings =   Building::active()
  ->whereIn('id', function ($query) {
    $query->select('un.building_id')
      ->from('receipts_generation as rg')
...
/**
 * Receipt types offered by the MERA Rent Receipt Report filter, keyed by the
 * receipts_generation_type stored on the receipt.
 * 0 tenant/rent, 1 general, 2 deposit - see RentReceiptGenerationController.
 */
const MERA_RECEIPT_TYPES = [
    '0' => 'Rent',
    '2' => 'Deposit',
    '1' => 'General',
];
...
public function showMeraRentReceiptReport(){
    $buildings = Building::active()->where('building_name', 'ILIKE', '%-MERA')->orderBy('building_name','asc')->get();
    $receiptTypes = self::MERA_RECEIPT_TYPES;
    return view('backoffice::Reports.mera_rent_receipt_report', compact('buildings','receiptTypes'));
}
...
public function meraRentReceiptReportDownload(Request $request){
    ...
    // Receipt Type filter. Whitelisted rather than trusted: the value goes
    // straight into the WHERE, and an unexpected one would silently widen the
    // report. Anything not recognised falls back to Rent, which is the default
    // in the dropdown and the behaviour this report had before the filter.
    $receiptType = (string) $request->input('receipt_type', '0');
    if (!array_key_exists($receiptType, self::MERA_RECEIPT_TYPES)) {
        $receiptType = '0';
    }
    $receiptTypeLabel = self::MERA_RECEIPT_TYPES[$receiptType];
    ...
    foreach ($buildingIds as $buildingId) {
        $rows = DB::select("
            ...
            WHERE rg.receipts_generation_status <> 2
              AND rg.receipts_generation_receipt_date BETWEEN ? AND ?
              AND rg.receipts_generation_type = ?
              AND rg.deleted_at IS NULL
              AND b.id = ?
            ORDER BY u.unit_no
        ", [$startDate, $endDate, $receiptType, $buildingId]);
        ...
        // Rent keeps the original file name so existing filing is undisturbed;
        // the other two are tagged so the files are not mistaken for rent.
        $typeSuffix = $receiptType === '0' ? '' : '_' . $receiptTypeLabel;

        if ($downloadType == 'pdf') {
            $fileName = $safeName . $typeSuffix . '_' . $monthName . '_' . $year . '.pdf';
            $pdf = \PDF::loadView('backoffice::Reports.mera_rent_receipt_pdf', $data)
                        ->setPaper('a4', 'landscape');
            $filePath = $tempDir . '/' . $fileName;
            $pdf->save($filePath);
        } else {
            $fileName = $safeName . $typeSuffix . '_' . $monthName . '_' . $year . '.xlsx';
            $filePath = $tempDir . '/' . $fileName;
    ...
    // Multiple buildings: create ZIP
    $zipFileName = 'MERA_' . $receiptTypeLabel . '_Receipt_' . $reportMonth->format('F_Y') . '.zip';
    $zipPath = $tempDir . '/' . $zipFileName;
    $zip = new ZipArchive();
    if ($zip->open($zipPath, ZipArchive::CREATE) === true) {
        foreach ($files as $file) {
