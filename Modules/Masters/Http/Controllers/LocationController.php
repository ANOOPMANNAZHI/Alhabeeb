...
class LocationController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('permission:change_status_location', ['only' => ['changeStatus']]);  
        $this->middleware('permission:view_location', ['only' => ['index','show']]);           
    }
    /**
    ...
    public function index(Request $request)
    {
        ...
        $locations = Location::when($fieldValue, function ($query) use($fieldValue,$fieldName){
                                        return $query->where($fieldName,'ilike', '%'.$fieldValue.'%')
                                        ;
                                       })->sortable()->paginate($noOfRecord);

        // $locations = Location::join('regions', 'regions.id', '=', 'locations.locations_region')
        ...
        $request->flash();                                        
        return view('masters::Location.list',compact('locations','fields','request'));
    }
    ...
    public function create()
    {
       ...
       $regions = Region::orderBy('region_name')->get();
        return view('masters::Location.add_edit',compact('regions'));
    }
    ...
    public function store(Request $request)
    {
       ...
       // $last = Location::orderBy('id', 'DESC')->first();
       // $lastid = $last['id'] + 1;
        ...
        //   'locations_region' => $request['locations_region'],
    ...
    $regions = Region::orderBy('region_name')->get();
        ...
        return view('masters::Location.add_edit',compact('location', 'previousUrl','backIdBreadCrumb','backUrlBreadCrumb','regions'));
    }
