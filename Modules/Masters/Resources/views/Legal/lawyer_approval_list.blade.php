...
@section('search_url', route('lawyerApproval')) 
@section('search_reset', route('lawyerApproval')) 
...
<div class="page-bar">
    <div class="page-title-breadcrumb">
        <div class=" pull-left">
            <div class="page-title">Approved Legal Cases for Review</div>
        </div>
        {{ Breadcrumbs::render('legalPlmsApproval') }}
    </div>
</div>
          ...
          <table class="table display product-overview mb-30" id="dtBasicExample">
            <thead>
                ...
                <tr>
                   ...
                   <input autocomplete="off" type="hidden" name="url_route" id="url_route"  value="{{route('lawyerApprovalSearch')}}" ></td>
               </tr>
           </thead>
           <tbody id="enquiry-search">

            @include('masters::Legal.lawyer_approval_list_ajax')  

        </tbody>
    </table>

    <div class="row "  id="pagination">               
    {{$lawyerApprovals->appends(\Request::except(['page','_token']))->links()}} 
    </div> 
...
<script>   
    ...
    $(document).ready(function() {
        ...
        /**********************************************************************************/
        $(document).on('click','.close_type', function(e) {        
            var legal_id = $(this).attr('data-id');
            var process_flow = $(this).attr('data-flow-id');
            /* if (confirm('Do you want to Approval Accept?')) {*/
                $.ajax({
