...
class UnitController extends Controller
{
    ...
    public function index(Request $request)
    {
	 ...
	 ->vacantUnitsByDays($request)
                    // Group by unit type only when the user has NOT asked for a
                    // sort. Applying it unconditionally made it the primary ORDER
                    // BY, so a clicked column could only order rows within a unit
                    // type and the header looked broken.
                    ->when(!$request->filled('sort'), function ($query) {
                        $query->orderBy('unit_type_id','asc');
                    })
                    ->sortable()
                    ->paginate($noOfRecord);
    ...
    */
    ...
    // This endpoint backs the inline column search on three screens that all
    // render masters::Unit.list - /unit, /comprehensiveUnits and /normalUnits -
    // so the management scoping is applied only for the two dashboard screens,
    // identified from the page the request came from. /unit must stay unscoped
    // or its list would silently lose rows the moment a search box is touched.
    ...
    else if($page == 'comprehensiveUnits'){
      $managementId = 1;
    }

    // filter() handles every search box the page sends - unit_code, unit_no,
    // building__building_name, unit_type_id, unit_vaccant_status and
    // unit_status - so the same base query as index() is used here. Previously
    // this method looked at unit_type_id and unit_vaccant_status only, and
    // forced unit_status = 1, which is why the other boxes did nothing and
    // Inactive could never be listed.
    $units = Unit::are()->filter($request)
     ->vaccatingUnits($request)
     ->vacantUnitsByDays($request)
     ->when($managementId !== null, function ($query) use ($managementId) {
        $query->whereHas('building', function ($query) use ($managementId) {
          $query->where('management_id', $managementId)
                ->where('building_status', 1);
        });
     })
     // Same rule as index(): only group by unit type when no sort was asked for.
     ->when(!$request->filled('sort'), function ($query) {
        $query->orderBy('unit_type_id','asc');
     })
     ->sortable()
