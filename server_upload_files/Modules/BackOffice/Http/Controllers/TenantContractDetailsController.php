    ...
    */
    public function allTenantsDetail(Request $request)
    {
		try{
			...
			$tenantIds = TenantContract::areFilter($request)->where('building_id',$building_id)->where('unit_id',$unit_id)->where('work_flow_processes_code',108)
				->orderByDesc('tenant_contract_status')
				->orderByDesc('tenant_contract_effective_date')
				->pluck('tenant_id')
				->unique()
				->values();
			$tenants = Tenant::whereIn('id',$tenantIds)->get()->sortBy(function($tenant) use ($tenantIds){
				return $tenantIds->search($tenant->id);
			})->values();
			//dd($tenants);
...
/**
    ...
    **/
    public function getBuildingDetails(Request $request)
    {
		...
		$tenantIds = TenantContract::areFilter()->where('unit_id',$unit_id)->where('work_flow_processes_code',108)
			->orderByDesc('tenant_contract_status')
			->orderByDesc('tenant_contract_effective_date')
			->pluck('tenant_id')
			->unique()
