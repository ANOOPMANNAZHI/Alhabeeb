            ...
            <div class="dataSearchBox">    
                <div class="row">
                    ...
                    <!-- 1 - Normal, 2- Bounce, 3 - Replace -->
                      ...
                      <input type="radio" class="pay_type" name="pay_type"  {{isset($rentReceiptInfo->receipts_generation_is_bounce_normal) && $rentReceiptInfo->receipts_generation_is_bounce_normal==3?'checked':''}}  value="3">
                      Replace
                    ...
                    <!-- ---------  If Replace radio selected: list PDCs without receipts ----   -->
                    <div class="col-sm-6" {{isset($rentReceiptInfo->receipts_generation_is_bounce_normal) && $rentReceiptInfo->receipts_generation_is_bounce_normal==3?'style=display:block':'style=display:none'}} id="pdc_replace_div">
                        <div class="form-group">
                            <label>Replace Cheque No: <small class="textRed">*</small></label>
                            <div class="p-relative">
                                <i class="fa fa-id-card-o icn-add" aria-hidden="true"></i>
                                <select name="pdc_replace_cheque" class="form-control" id="pdc_replace_cheque">
                                    @if(isset($rentReceiptInfo->receipts_generation_is_bounce_normal) && $rentReceiptInfo->receipts_generation_is_bounce_normal==3 && isset($contract_replace_pdc))
                                        @foreach($contract_replace_pdc as $pdc)
                                            <option value="{{$pdc->id}}" data-cheque="{{$pdc->pdc_check_no}}" {{($pdc->id==$rentReceiptInfo->receipts_generation_is_pdc_bounce_id)?'selected':''}}>{{$pdc->pdc_check_no}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
...
$(document).on('click','.pay_type',function(){
    ...
    // Drop any Replace remark left over from a previous choice. Switching from
    // Replace to Bounce or Normal used to keep the old line, which is how a
    // Bounce receipt ended up saved with "Received cash payment for cheque no"
    // and no number. Only the auto-generated line is cleared; anything the
    // operator typed themselves is left alone.
    if(cash_type != 3){
        var $remark = $('#receipts_generation_remark');
        if(/^Received cash payment for cheque no/i.test($remark.val() || '')){
            $remark.val('');
        }
    }
    ...
    if(cash_type == 1){ // Normal
            ...
            $("#pdc_bounce_div").hide();
            $("#pdc_replace_div").hide();
            $("#replace_cheque_no_div").hide();
            $("#pdc_replace_cheque").removeAttr('required');
            $("#replace_cheque_no").removeAttr('required');
    }
    ...
    else if(cash_type == 3){ // Replace

        var contractId = $("#tenant_contract_no").val();
        $("#pdc_bounce_cheque").removeAttr('required');
        $("#pdc_bounce_div").hide();
        $("#pdc_replace_cheque").attr('required','required');
        $("#pdc_replace_cheque").val('');
        if(contractId == ''){
            $("#pdc_replace_cheque").html('<option value="">Select the Agreement No first</option>');
        } else {
            $.ajax({
                method: "POST",
                url: "{{route('pdcListWithoutReceiptByContract')}}",
                data: { contractId: contractId, "_token": $('meta[name="csrf-token"]').attr('content') },
                success: function(data){
                    var parsed_data = JSON.parse(data);
                    $('#pdc_replace_cheque').html(parsed_data.option_html);
                    // The list always opens on the "Select Cheque" placeholder,
                    // which carries no data-cheque, so writing the remark here
                    // saved the bare phrase with no number. Only set it once a
                    // cheque is genuinely selected.
                    setReplaceChequeRemark();
                }
            });
        }
        $("#pdc_replace_div").show();
    }

});

/**
 * Write the Replace remark from the cheque currently selected.
 *
 * Writes nothing while the placeholder option is selected - that option has no
 * data-cheque, and setting the line anyway is what saved receipts reading
 * "Received cash payment for cheque no" with the number missing. Any stale line
 * left over from an earlier selection is cleared instead.
 */
function setReplaceChequeRemark(){
    var chequeNo = $('#pdc_replace_cheque').find('option:selected').data('cheque');
    var $remark  = $('#receipts_generation_remark');

    if(chequeNo === undefined || chequeNo === null || String(chequeNo) === ''){
        if(/^Received cash payment for cheque no/i.test($remark.val() || '')){
            $remark.val('');
        }
        return;
    ...
    $remark.val('Received cash payment for cheque no ' + chequeNo);
}

$(document).on('change', '#pdc_replace_cheque', function(){
    setReplaceChequeRemark();
});
