...
<div id="items">
  ...
  <div class="item-row">
      ...
      @else
        @php
          $storedText = trim((string) ($receiptInfo->receipts_generation_remark ?: $receiptInfo->receipts_generation_description));

          // Deposit receipts (type 2) store this line as free text captured by
          // the entry form's javascript. When the form could not resolve the
          // unit - it returns an array instead of a single unit whenever the
          // lookup matches more than one - the unit number was left out and the
          // print came out as "DEPOSIT FOR UNIT NO.  <building>". The contract
          // itself always knows the unit, so derive the line from live data
          // whenever the stored text does not already name the right unit.
          if ((string) $receiptInfo->receipts_generation_type === '2') {
              $depositContract = $receiptInfo->tenantContractInfo;
              $depositUnitNo   = $depositContract && $depositContract->Unit ? trim((string) $depositContract->Unit->unit_no) : '';
              $depositBldg     = $depositContract && $depositContract->building ? trim((string) $depositContract->building->building_name) : '';

              $namesTheUnit = $depositUnitNo !== ''
                  && $storedText !== ''
                  && stripos($storedText, $depositUnitNo) !== false;

              if ($depositUnitNo !== '' && !$namesTheUnit) {
                  $storedText = trim('DEPOSIT FOR UNIT NO. ' . $depositUnitNo . ' ' . $depositBldg);
              }
          }
        @endphp
        {{ $storedText }}
