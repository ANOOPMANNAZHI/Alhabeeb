...
<div class="page-bar">
  <div class="page-title-breadcrumb">
    <div class=" pull-left">
      <div class="page-title">MERA Rent Receipt Report</div>
    </div>
    {{ Breadcrumbs::render('showMeraRentReceiptReport') }}
  </div>
</div>
<div class="row">
  <div class="col">
    <div class="card card-box salesSearchBox">
        @can('view_mera_rent_receipt_report')
      <form action="{{route('meraRentReceiptReportDownload')}}" method="POST" id="form_sample_2" class="form-horizontal" enctype="multipart/form-data" data-toggle="validator">
        {{csrf_field()}}
          ...
          <div class="row">
            <div class="col-sm-6">
              <div class="form-group">
                <label for="report_month">Month<small class="textRed">*</small></label>
                <div class="p-relative">
                  <i class="fa fa-calendar icn-add" aria-hidden="true"></i>
                  <input type="month" class="form-control" id="report_month" name="report_month" value="{{ date('Y-m') }}" required>
                </div>
              </div>
            </div>
            <div class="col-sm-6">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                <label for="receipt_type">Receipt Type<small class="textRed">*</small></label>
                <div class="p-relative">
                  <i class="fa fa-file-text-o icn-add" aria-hidden="true"></i>
                  {{-- Defaults to Rent, so leaving this alone reproduces the
                       report exactly as it behaved before the filter existed. --}}
                  <select class="form-control" id="receipt_type" name="receipt_type" required>
                    @foreach($receiptTypes as $typeValue => $typeLabel)
                    <option value="{{ $typeValue }}" {{ $typeValue === '0' ? 'selected' : '' }}>{{ $typeLabel }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
