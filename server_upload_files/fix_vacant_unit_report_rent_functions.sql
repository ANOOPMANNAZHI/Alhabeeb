...
-- Equivalent of database/migrations/2026_09_02_000000_fix_vacant_unit_report_rent_functions.php
--
-- Fixes the "Report on Vacant Unit" Rent (Per Month) figure. For a unit with
-- more than one terminated contract, the old function picked contract_rent
-- via an independent MAX(tenant_contract_rent) grouped by unit -- unrelated
-- to which contract actually had the latest termination_date. Dates were
-- correct but rent could come from an old, unrelated contract (e.g. Al
-- Khuwair House unit 33 showed 300 from a contract terminated in 2020
-- instead of 225 from the contract actually terminated 2026-06-30).
--
-- Fix: DISTINCT ON (u.id) ... ORDER BY termination_date DESC, tr.id DESC so
-- every column comes from the same (latest) terminated contract row.
