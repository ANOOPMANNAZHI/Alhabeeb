...
/**
 * "Termination Dues" under PLM Module -> Operations -> Tenant Termination, two team permissions
...
class AddTerminationDuesMenu extends Migration
{
    ...
    public function up()
    {
        ...
        $operations = $plmModule
            ...
            : null;
        $tenantTermination = $operations
            ? DB::table('menu')->where('menu_name', 'Tenant Termination')->where('parent_menu', $operations->id)->where('menutype', 1)->first()
            : null;
        $parentMenuId = $tenantTermination ? $tenantTermination->id : ($operations ? $operations->id : 0);
